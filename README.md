...
> :bulb: Windows-ympäristössä kirjaston asentaminen saattaa vaatia skriptien suorituksen sallimisen Powershell-ympäristössä. Sen voi tehdä pääkäyttäjänä antamalla Powershell-komennon `Set-ExecutionPolicy RemoteSigned`. Myös VisualStudio Code:n käyttämät lisäosat saattavat vaatia tämän komennon suorittamisen. Se sallii koneelle tallennettujen PS-skriptien suorittamisen. Myös verkon kautta ladatut skriptit suoritetaan, mikäli niissä on sähköinen allekirjoitus. Jos halutaan sallia vain allekirjoitettujen skriptien suoritus, annetaan komento `Set-ExecutionPolicy AllSigned`. Rajoituksen voi myös poistaa kokonaan komennolla `Set-ExecutionPolicy Unrestricted`, mutta tämän komennon käyttämimen on potentiaalinen tietoturvariski.
